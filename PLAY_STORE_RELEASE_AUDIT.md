...
App: **Photo Compressor & BG Remover**
...
The direct Save, Share, Open, Remove, and Clear History actions remain ungated, and the custom native layout renders both localized ad attribution and AdChoices. The former History-entry interstitial has been moved to Back from a nonempty `HistoryUiState.Content` after actual session engagement through a list scroll or History-item interaction: an eligible loaded ad is shown before pop/navigation while History remains visible, then navigation continues from its completion callback; populated enter-and-immediate-Back and empty, loading, or error exits do not offer it. A stable, non-cancelled completed batch summary with at least three result rows and at least one success can now add a final inline native item after substantive per-image results; active, short, all-failed, and cancelled batches are excluded, and the placement is hidden during full-screen ads. These changes improve the general unexpected-ad, obstruction, and accidental-click posture but still require rendered-flow validation and are not a guarantee of Play approval. The verified Ages 13–15, Ages 16–17, and Ages 18-and-over audience requires a Families-policy analysis and applicable implementation updates before production. Target SDK, model provenance, production AdMob identifiers, and on-device image processing remain in good shape; localization and native-speaker review must be current before publishing localized listings.

| Status | Finding |
| --- | --- |
| **Release blocker — 13+ audience treatment** | Home, Progress, Result, Background, and History can each render more than one ad placement on a page; eligible completed Progress can combine top and bottom banners with a final native item. Google's prohibition on multiple ad placements on a page is a **Families** ad-format requirement, not an unconditional general Made for Ads rule. The verified audience includes Ages 13–15 and Ages 16–17, which can be treated as children in some locales. Apply the corresponding Families ad-serving and layout treatment wherever required before production. |
| **Release blocker — 13+ audience treatment** | The current build lacks a neutral age screen and child/teen or under-age ad-request treatment, declares Advertising ID access, and includes app-open ads, which AdMob says are ineligible for Families-compliant apps. The verified audience therefore affects the entire ad implementation, not only banner density. |
...
| **Blocker** | The privacy policy now contains the verified publisher identity and privacy contact, but it still needs an active, public, non-geofenced, non-editable HTTPS webpage URL (not a PDF). |
...
- Users can remove visible History records, clear Android app storage/cache, and delete saved gallery files through the gallery/file manager. Hidden stale History metadata can require Clear Storage. Those actions do not control Google's retention.
...
### Blocker — policy hosting

`PRIVACY_POLICY.md` now identifies Naimish Gupta, provides
naimish.app@gmail.com, uses the verified application name, and is synchronized
with the in-app English policy. The policy describes the current implementation,
Google Mobile Ads automatic collection/sharing, TLS, UMP, the four ad formats,
local History/cache behavior, persisted URI access, the legacy API 28 save
permission, the on-device model, and the lack of accounts. Before publication,
the publisher must still host it on an active, public, non-geofenced,
non-editable HTTPS webpage (not a PDF) and use that URL in Play Console.

The formerly translated policy resources were removed so every locale now
falls back to the same verified English policy. Native-language policy
translations require legal and native-speaker review before they are restored.
...
| --- | --- | --- |
| Locale validation | Fresh `:app:generateDebugLocaleConfig` and `tools/check_localizations.py` runs passed | Current-tree run reported 25 non-English locales, 232 translated strings, and one plural resource; all 27 base/localized XML catalogs parsed, and `values-id`/`values-in` are byte-identical. The verified policy is intentionally non-translatable pending legal/native-language review. |
...
The searchable in-app selector exposes 26 packaged language or regional options including English, plus System Default. Debug pseudo locales are enabled separately. The current-tree `tools/check_localizations.py` run passed all 25 non-English locale packs with 232 translated strings and one plural resource; all XML catalogs parsed, and the Indonesian alias remains byte-identical. The privacy-policy content deliberately falls back to the verified English source in every locale until legal and native-language review is completed. Repeat validation after every catalog change. This validates repository structure/content rules only; it is not human linguistic review. Publish localized store listings only after native-speaker review, and use language-matched screenshots when screenshots contain text.

## Production Play And AdMob Checklist

All items below remain unchecked unless the publisher supplies external evidence.

- [ ] Configure the verified intended groups Ages 13–15, Ages 16–17, and Ages 18 and over only after the advertising implementation supports them. Because 13–15 and 16–17 can be treated as children in some locales, implement the required neutral age treatment, child/teen or under-age request handling, identifier restrictions, non-personalized age-appropriate ads, eligible self-certified SDK/source and content-rating controls, and Families-compliant layouts; also resolve AdMob's Families ineligibility for app-open ads wherever applicable. References: <https://support.google.com/googleplay/android-developer/answer/9867159>, <https://support.google.com/googleplay/android-developer/answer/9893335>, and <https://support.google.com/admob/answer/9620632>.
...
- [ ] Host the completed privacy policy as an active, public, non-geofenced, non-editable HTTPS webpage (not a PDF) and enter the same URL in Play Console.
