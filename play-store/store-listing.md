...
The longer Play Store title adds a truthful feature descriptor for discovery;
the shorter launcher label matches the current Android resources and graphics.
...
It displays the shorter `Photo Compressor` launcher brand and accurately names
batch compression, resizing, conversion, and background removal.
...
Photo Compressor helps you reduce image size, resize photos, convert image formats, and remove or replace backgrounds from selected images.
...
| Target audience | **TODO — publisher must choose the actual intended age groups, accounting for who is considered a child under applicable local laws; keep the privacy policy, ads configuration, and store content consistent with that decision.** |
...
- [ ] **Target audience and content:** complete the publisher's intended-age selection and account for applicable local definitions of a child. If any selected audience includes children or ads are served to users of unknown age, the current build is not ready unchanged: multiple placements such as the completed Progress top/bottom/native combination are prohibited for those ad requests. Assess a neutral age screen, child/unknown-age request treatment and AAID restrictions, non-personalized age-appropriate ads, eligible self-certified SDK sources and content-rating controls, every multi-placement layout, and AdMob's Families ineligibility for app-open ads. Answer all resulting Families/ads questions truthfully.
